...
const FaceRegistrationPage: React.FC = () => {
	...
	const faceRegKey = user ? `face-reg-done-${user.id}` : null;
	...
	// Track when user has attempted — face detected at least once
	useEffect(() => {
		if (!hasTried && (faceStatus === 'detected' || faceStatus === 'capturing')) {
			setHasTried(true);
		}
	}, [faceStatus, hasTried]);
	...
	const handleSkip = useCallback(() => {
		if (faceRegKey) localStorage.setItem(faceRegKey, '1');
	...
	}, [isOnboarding, faceRegKey, navigate]);
	...
	const showSkip = isOnboarding && hasTried && phase !== 'success' && phase !== 'submitting';
