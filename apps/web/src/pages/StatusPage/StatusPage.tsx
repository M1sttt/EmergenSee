...
export default function StatusPage() {
	...
	const displayUsers = useMemo(() => {
		...
		if (effectiveSelectedDeptId !== consts.allDeptsValue) {
			filteredUsers = filteredUsers.filter(user =>
				user.departments?.some(departmentId => getEntityId(departmentId) === effectiveSelectedDeptId),
			);
		}
			...
			},
			{
				...
				renderCell: ({ status }) =>
					status && status.status !== ResponderStatus.UNKNOWN ? (
						...
						<Badge tone="neutral">
							{RESPONDER_STATUS_LABELS[ResponderStatus.UNKNOWN as keyof typeof RESPONDER_STATUS_LABELS] ||
					...
					const canReport =
						...
						(isDeptAdmin &&
							(user.departments || []).some(departmentId =>
								userAdminDepts.map(getEntityId).includes(departmentId),
							));
	...
	return (
		<div className="ui-page">
					...
					{(isGlobalAdmin || isDeptAdmin) && (
						<div className="flex items-center gap-2">
							...
							<SelectDropdown
								...
								onChange={value =>
									setSelectedDeptId(Array.isArray(value) || !value ? consts.allDeptsValue : value)
								}
			...
			{!effectiveSelectedEventId ? (
				<div className="ui-empty-state">{strings.noActiveEvents}</div>
			) : (
				<GenericTable
