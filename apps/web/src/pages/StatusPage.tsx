...
export default function StatusPage() {
  ...
  const { data: events = [], isLoading: isLoadingEvents } = useQuery({ queryKey: ['events'], queryFn: eventsService.getAll });
  const { data: deptsResponse, isLoading: isLoadingDepts } = useQuery<any>({ queryKey: ['departments'], queryFn: departmentsService.getAll });
  const { data: users = [], isLoading: isLoadingUsers } = useQuery({ queryKey: ['users'], queryFn: usersService.getAll });
  const { data: statusUpdates = [], isLoading: isLoadingStatus } = useQuery({ queryKey: ['status'], queryFn: statusService.getAll });

  const activeEvents = useMemo(() => events.filter(e => e.status === EventStatus.ONGOING), [events]);
  const departments = useMemo(() => Array.isArray(deptsResponse) ? deptsResponse : (deptsResponse?.data || []), [deptsResponse]);
  ...
  const validDepartments = isGlobalAdmin 
    ? departments 
    : isDeptAdmin ? userAdminDepts : [];
  ...
  const displayUsers = useMemo(() => {
    ...
    if (selectedDeptId !== 'all') {
      ...
      if (!isGlobalAdmin && isDeptAdmin) {
        ...
        filteredUsers = users.filter((u: any) => u.departments?.some((dId: string) => adminDeptIds.includes(dId)));
      }
    }
    ...
    return filteredUsers.map((u: any) => {
      ...
      userStatuses.sort((a:any, b:any) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {displayUsers.map(({ user, status }) => {
                const canReport = isGlobalAdmin || (isDeptAdmin && user.departments.some((ud: string) => userAdminDepts.map((d:any) => d.id || d._id).includes(ud)));
                  ...
                  <tr key={user.id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      {status && status.status !== ResponderStatus.UNKNOWN ? (
                         <span className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${status.status === ResponderStatus.SAFE ? 'bg-green-100 text-green-800' : status.status === ResponderStatus.NEED_HELP ? 'bg-red-100 text-red-800' : 'bg-gray-100 text-gray-800'}`}>
                           {RESPONDER_STATUS_LABELS[status.status as keyof typeof RESPONDER_STATUS_LABELS] || status.status}
                         </span>
                      ) : (
                         <span className="px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-gray-100 text-gray-800">
                           {RESPONDER_STATUS_LABELS[ResponderStatus.UNKNOWN as keyof typeof RESPONDER_STATUS_LABELS] || 'Unknown'}
                         </span>
                      )}
                    </td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      {canReport && (
                        <div className="flex justify-end gap-2">
                          <button 
                            onClick={() => reportMutation.mutate({ status: ResponderStatus.SAFE, userId: user.id, eventId: selectedEventId })}
                            className="bg-green-50 text-green-700 hover:bg-green-100 px-3 py-1 rounded text-xs font-medium transition"
                          >
                            Mark Safe
                          </button>
                          <button 
                            onClick={() => reportMutation.mutate({ status: ResponderStatus.NEED_HELP, userId: user.id, eventId: selectedEventId })}
